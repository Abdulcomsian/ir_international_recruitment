...
@section('content')
    <div class="container">
        <h1>Create New Navigation ({{ $quebecLegalAspect->title }})</h1>
        <form action="{{ route('quebec.legal-aspects.navigations.store',$quebecLegalAspect->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <button type="submit" class="btn btn-success">Create Navigation</button>
        </form>
    </div>
@endsection
